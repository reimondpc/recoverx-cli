"""Tests for the PNG file carver."""

from __future__ import annotations

import pytest

from recoverx.core.carving.png import PNGCarver

_HEADER = b"\x89PNG\r\n\x1a\n"
_FOOTER = b"\x00\x00\x00\x00IEND\xae\x42\x60\x82"
_MIN_SIZE = 67  # matches signatures.py


def _make_png_chunk(size: int, fill: int = 0xA5) -> bytes:
    body_size = max(size - len(_HEADER) - len(_FOOTER), 0)
    return _HEADER + bytes([fill] * body_size) + _FOOTER


@pytest.fixture
def carver():
    return PNGCarver()


class TestPNGCarver:
    def test_carve_finds_single_png(self, carver):
        chunk = _make_png_chunk(_MIN_SIZE)
        data = b"\x00" * 100 + chunk + b"\x00" * 100
        results = carver.carve(data)
        assert len(results) == 1
        assert results[0].offset_start == 100
        assert results[0].offset_end == 100 + len(chunk)
        assert results[0].signature_name == "PNG"
        assert results[0].extension == "png"

    def test_carve_no_png(self, carver):
        assert carver.carve(b"\x00" * 1024) == []

    def test_carve_multiple_pngs(self, carver):
        c1 = _make_png_chunk(_MIN_SIZE, 0xAA)
        c2 = _make_png_chunk(_MIN_SIZE, 0xBB)
        data = b"\x00" * 50 + c1 + b"\x00" * 30 + c2 + b"\x00" * 50
        results = carver.carve(data)
        assert len(results) == 2
        assert results[0].offset_start == 50
        assert results[1].offset_start == 50 + len(c1) + 30

    def test_carve_min_size_filter(self, carver):
        data = _HEADER + b"\x01" * 10 + _FOOTER
        assert len(carver.carve(data)) == 0

    def test_carve_no_footer_skips(self, carver):
        assert len(carver.carve(_HEADER + b"\x01" * 200)) == 0

    def test_carve_empty_data(self, carver):
        assert carver.carve(b"") == []

    def test_carve_data_integrity(self, carver):
        chunk = _make_png_chunk(_MIN_SIZE)
        data = b"\x00" * 50 + chunk + b"\x00" * 50
        results = carver.carve(data)
        assert results[0].data == chunk
